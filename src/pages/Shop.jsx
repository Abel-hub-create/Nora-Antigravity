...
const BOOSTERS = [
  { type:'one',  labelKey:'shop.boosterOne',  descKey:'shop.boosterOneDesc',  price:100,  count:1  },
  { type:'five', labelKey:'shop.boosterFive', descKey:'shop.boosterFiveDesc', price:400,  count:5  },
  { type:'ten',  labelKey:'shop.boosterTen',  descKey:'shop.boosterTenDesc',  price:800,  count:10 },
];
...
function BoosterCard({ booster, onOpen, loading }) {
  ...
  return (
    <motion.button
      ...
      disabled={!!loading}
      className="relative bg-surface/60 border border-white/10 rounded-2xl p-4 flex flex-col items-center gap-3 hover:border-primary/30 transition-colors w-full disabled:opacity-60"
    >
      <motion.img
        src="/boosternorafront.png"
        alt="Booster"
        style={{ width:64, height:'auto', objectFit:'contain', filter:'drop-shadow(0 4px 12px rgba(168,139,250,0.4))' }}
        animate={active ? { rotate:[0,10,-10,10,-10,0] } : { y:[0,-4,0] }}
        transition={active
      ...
      <div className="flex items-center gap-1.5 bg-amber-500/15 border border-amber-500/30 rounded-full px-3 py-1">
        {active ? <Loader2 size={11} className="animate-spin text-amber-400" /> : <CoinIcon size={13} />}
        <span className="text-amber-300 text-xs font-bold">{booster.price}</span>
      </div>
    </motion.button>
  );
}
...
export default function Shop() {
  ...
  const [revealPacks, setRevealPacks]  = useState(null);
  ...
  return (
    <div className="p-6 pt-8 pb-28 space-y-8 max-w-lg mx-auto">
      ...
      {revealPacks && (
        <BoosterOpenModal packs={revealPacks} onClose={() => setRevealPacks(null)} />
      )}
      ...
      <section>
        <div className="flex items-center gap-2 mb-4">
          <img src="/boosternorafront.png" alt="" style={{ width:18, height:'auto', objectFit:'contain' }} />
          <h2 className="text-sm font-bold text-text-main uppercase tracking-wider">{t('shop.boosters')}</h2>
        </div>
        <div className="grid grid-cols-3 gap-3">
          {BOOSTERS.map(b => (
            <BoosterCard key={b.type} booster={b} onOpen={handleOpenBooster} loading={openLoading} />
          ))}
        </div>
      </section>
